...
### 2026-09-03 社群分享主圖更新（Codex）

- 舊 `og-image.jpg` 的陌生女性與仿 LINE／電話按鈕已移除；新圖改為老闆夫妻的專業清潔形象、實際清潔工具與明亮居家場景，並以固定字型疊加品牌與服務範圍，避免生成式文字錯字。
- 正式 OG／Twitter／JSON-LD 改指向 `og-cleaning-v4.jpg`（1200×630），利用新檔名避開 LINE 等平台對舊圖片網址的長時間快取；`og-image.jpg` 保留同圖作舊連結相容。服務列使用兩列、每列三欄的等寬置中配置，列出居家清潔、大掃除、裝潢細清、公家機關、學校與公司行號；服務地區前方使用獨立定位圖示。文字與左側安全區同步放大，照片漸層向右延伸，確保社群縮圖仍可辨識。
- 分享圖只負責小尺寸辨識品牌、居家清潔與服務範圍，不內嵌電話或假按鈕。`scripts/prepare-pages-artifact.sh` 已把版本化 OG 圖列為必要部署檔。
